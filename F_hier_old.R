...
#load old data
o_data <- read_csv("/work/KatFred/Andreas/data/old_IC.csv")
#trial_data <- read_csv("/work/KatFred/Andreas/o_data_trial.csv")
...
ID <- unique(o_data$ID)
nsubs <- length(ID)
nblocks <- length(unique(o_data$Block))
ndecks <- 4
ntrials_max <- 100
deck_cols <- c("Acount", "Bcount", "Ccount", "Dcount")
# all outcomes (NetScore)
NetScore_scaled <- o_data$NetScore/4 #scaled
...
for (s in 1:nsubs) {
  ...
  for (d in 1:ndecks) {
    counts_all[s,,d] <- unlist(o_data[o_data$ID==ID[s], deck_cols[d]], use.names = FALSE)
  }
  ...
  NetScore_all[s,] <- NetScore_scaled[o_data$ID==ID[s]] 
...
nblocks <- 5
ndecks <- 4
participant_ids <- unique(o_data$ID)
...
for (i in seq_along(participant_ids)) {
  ...
  participant_data <- o_data[o_data$ID == participant_id, ]
...
score_weight <- score_weight_all[1, ,]
...
samples <- jags.parallel(data, inits=NULL, params,
                         ...
                         n.chains=3, n.iter=25000, n.burnin=5000, n.thin=1, n.cluster=4)
